...
use farcaster::diff_plan::{DiffLayout, DiffPlanOptions, plan_patch};
...
fn plan_metrics(plan: &farcaster::diff_plan::DiffRenderPlan) -> PlanMetrics {
    let mut metrics = PlanMetrics::default();
    for file in &plan.files {
        metrics.rows += file.rows.len();
        for row in &file.rows {
            let farcaster::diff_plan::DiffPlanRow::Line(line) = row else {
                continue;
            };
...
fn planned_or_panic(patch: &str, options: DiffPlanOptions) -> farcaster::diff_plan::DiffRenderPlan {
