...
directory. Linux and macOS desktop packaging starts the user's login shell in
that directory and captures its environment before spawning Pi directly. This
keeps the environment and global/project settings, context, skills, prompt
templates, packages, and extensions aligned with the terminal client without a
second launcher. Do not inject product prompts or bypass Pi.
