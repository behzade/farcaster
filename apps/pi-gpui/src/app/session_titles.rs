...
use super::{PiApp, SessionTitleEdit};
use crate::{runtime::RuntimeCommand, sessions::normalize_session_path};

impl PiApp {
    pub(super) fn begin_session_title_edit(
        &mut self,
        path: PathBuf,
        project: PathBuf,
        title: String,
    ...
    pub(super) fn commit_session_title_edit(
        &mut self,
        _window: &mut Window,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(super) fn cancel_session_title_edit(&mut self, cx: &mut Context<Self>) {
