...
pub(super) const MARKDOWN_CHUNK_HARD_BYTES: usize = 8 * 1024;
...
pub(super) struct MarkdownChunk {
    ...
    pub(super) end: usize,
    pub(super) fence: Option<FenceContinuation>,
}
...
pub(crate) struct TranscriptRowUpdate {
    pub(super) rows: Option<PersistentVec<TranscriptRow>>,
    pub(super) unchanged_prefix_rows: usize,
}
...
#[cfg(test)]
pub(super) fn matching_item_prefix(
    previous_items: &(impl Indexed<Arc<TranscriptItem>> + ?Sized),
    items: &(impl Indexed<Arc<TranscriptItem>> + ?Sized),
) -> (usize, usize) {
...
pub(super) fn markdown_chunks(text: &str) -> Vec<MarkdownChunk> {
...
pub(super) fn markdown_chunk_text(text: &str, chunk: MarkdownChunk) -> Cow<'_, str> {
...
pub(super) fn markdown_fence_closes(line: &str, fence: MarkdownFence) -> bool {
    let indent_len = line.bytes().take_while(|byte| *byte == b' ').count();
    if indent_len > 3 {
        return false;
    }
...
pub(super) fn invocation_kind(display: &str, resolved: &str) -> &'static str {
    let count = display
        .split_whitespace()
        .filter(|token| {
            token
...
pub(super) fn is_mixed_invocation_message(display: &str, resolved: &str) -> bool {
...
pub(super) fn highlighted_invocation_markdown(display: &str, resolved: &str) -> String {
...
pub(super) fn thinking_has_details(item: &TranscriptItem) -> bool {
