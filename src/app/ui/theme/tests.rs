...
fn bundled_session_rows_have_room_for_two_lines_and_padding() {
    for definition in BUILT_IN_THEMES.iter() {
        ...
        // Session and draft rows stack a title and metadata, with a 2px gap
        // and 4px padding above and below. Metadata includes an inline icon.
        ...
        assert!(
            theme.layout.session_row_height >= required,
            "{}",
            definition.name
        );
        assert!(
