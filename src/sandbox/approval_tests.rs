...
fn setup() -> Result<
    ...
    String,
> {
    setup_with_nono(super::super::test_nono_bypass())
}

fn setup_with_nono(
    nono: super::super::NonoExecutable,
) -> Result<
    (
        tempfile::TempDir,
        ApprovalService,
...
async fn rejected_nono_profile_is_not_activated_or_persisted() -> Result<(), String> {
    ...
    let (root, service, ui, project, _home) =
        setup_with_nono(super::super::NonoExecutable::Fixed(nono))?;
    ui.set_project_trusted(true);
    let request = tokio::spawn(async move {
        service
            .request_access(RequestAccessParams {
