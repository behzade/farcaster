...
) -> Result<crate::adapter::main_session::MainSessionMetadata, String> {
...
pub fn spawn_main(
    ...
    (
        ...
        String,
        crate::adapter::main_session::MainSessionMetadata,
    ),
    String,
> {
    let mut prepared = command.command(&launch.project)?;
...
> {
    ...
    let thread = match &launch.start {
        crate::SessionStart::New => connection.start_thread(&cwd, None, None, access_mode)?,
        crate::SessionStart::Resume(_) => {
...
fn load_main_metadata(
    ...
    project: &std::path::Path,
) -> Result<(crate::adapter::main_session::MainSessionMetadata, Skills), String> {
...
impl CodexWorkerSession {
    ...
    fn child_execution(&mut self, child: &str) -> crate::WorkerModelSelection {
        ...
        match super::catalog::stored_identity(&self.codex_home, child) {
            ...
            Ok(None) => crate::WorkerModelSelection::default(),
            Err(error) => {
                zlog::warn!("Codex child identity unavailable for {child}: {error}");
                crate::WorkerModelSelection::default()
            }
        }
    }
...
fn configure_codex_app_server(command: &mut std::process::Command, mode: crate::HarnessAccessMode) {
...
fn configure_farcaster_mcp(
    ...
    if access_mode == crate::HarnessAccessMode::Full {
        command
            .arg("-c")
...
fn codex_input_delivery(item: &Value) -> Option<WorkerActivity> {
    ...
    let message = content
        ...
        .collect::<Vec<_>>()
        .join("\n");
    let images: Vec<crate::extensions::PromptImage> = content
        .into_iter()
...
fn codex_account_usage(limits: &Value) -> crate::AccountUsage {
    ...
    let weekly = ["primary", "secondary"]
        ...
        .and_then(|window| {
            crate::AccountUsageWindow::from_used_percent(
                window["usedPercent"].as_f64()?,
                window["resetsAt"].as_i64(),
            )
        });
    crate::AccountUsage { weekly }
}

fn codex_child_event_outcome(item: &Value) -> Option<crate::ChildSessionOutcome> {
    match item.get("kind").and_then(Value::as_str) {
        Some("completed") => Some(crate::ChildSessionOutcome::Complete),
        Some("failed") => Some(crate::ChildSessionOutcome::Failed),
        Some("interrupted") => Some(crate::ChildSessionOutcome::Incomplete),
        _ => None,
    }
}

fn codex_child_thread_outcome(thread: &Value) -> Option<crate::ChildSessionOutcome> {
    if thread.pointer("/status/type").and_then(Value::as_str) == Some("systemError") {
        return Some(crate::ChildSessionOutcome::Failed);
    }
    match thread
        .get("turns")
        .and_then(Value::as_array)
