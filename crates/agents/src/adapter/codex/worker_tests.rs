...
fn delivered_inputs_preserve_images() {
    use crate::extensions::PromptImage;
    ...
    for (kind, running) in [
        ...
        if kind != "interacted" {
            let outcome = match kind {
                "completed" => Some(crate::ChildSessionOutcome::Complete),
                "failed" => Some(crate::ChildSessionOutcome::Failed),
                "interrupted" => Some(crate::ChildSessionOutcome::Incomplete),
                _ => None,
            };
            assert_eq!(
                session.poll(),
                Some(WorkerEvent::Activity(
                    WorkerActivity::ChildSessionsChanged {
                        ...
                        outcome,
                        execution: Some(crate::WorkerModelSelection::default()),
                    }
                ))
            );
        }
...
fn correlated_codex_control_rejection_reaches_the_session_caller() {
    ...
    use crate::{SessionCommand, SessionEvent, SessionOperation, SessionTransport};
...
fn malformed_success_is_delivery_unknown_for_every_prompt_mode() {
    ...
    use crate::extensions::PromptMode;
    use crate::{SessionCommand, SessionEvent, SessionResponseErrorKind, SessionTransport};

    fn project(
        transport: &mut WorkerSessionTransport,
        conversation: &mut ConversationState,
        responses: &mut Vec<crate::SessionResponse>,
...
fn malformed_native_reply_stays_correlatable_but_never_joins_next_handoff() {
    ...
    session
        .submit_prompt(
            ...
            "old unknown".into(),
            WorkerSendMode::Steer,
            vec![crate::extensions::PromptImage::new(
                "AQID".into(),
                "image/png".into(),
            )],
        )
...
fn native_queue_stays_visible_across_turn_completion_until_delivery() {
    ...
    use crate::extensions::PromptMode;
    use crate::{SessionCommand, SessionEvent, SessionTransport};
