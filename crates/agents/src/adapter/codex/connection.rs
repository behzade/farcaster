...
impl<R: BufRead, W: Write> CodexConnection<R, W> {
    ...
    pub fn send_request(&mut self, method: &str, params: Value) -> Result<CodexRequestId, String> {
        self.next_id = self
            .next_id
            .checked_add(1)
    ...
    pub fn send_notification(&mut self, method: &str, params: Option<Value>) -> Result<(), String> {
        self.writer
    ...
    pub fn respond(&mut self, id: &CodexRequestId, result: Value) -> Result<(), String> {
        self.writer
    ...
    pub fn next(&mut self) -> Result<CodexInbound, String> {
    ...
    pub fn wait_response<T: DeserializeOwned>(&mut self, id: &CodexRequestId) -> Result<T, String> {
    ...
    pub fn initialize_experimental(
        &mut self,
        client: CodexClientInfo,
    ) -> Result<CodexInitializeResponse, String> {
    ...
    pub fn interrupt_turn(&mut self, thread_id: &str, turn_id: &str) -> Result<(), String> {
        let id = self.send_request(
            "turn/interrupt",
    ...
    pub fn into_parts(self) -> (R, W, VecDeque<CodexInbound>, i64) {
