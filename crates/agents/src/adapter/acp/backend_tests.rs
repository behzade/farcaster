use super::*;
use crate::adapter::{self, antigravity::PROFILE as ANTIGRAVITY};
use std::path::Path;

#[test]
fn external_agents_have_registered_workers_and_valid_session_identities() {
    let (workers, _) = adapter::worker_factories(crate::AgentLaunchConfig::default());
    ...
    adapter::session_storage::validate_session_locator(profile.backend, &path)
