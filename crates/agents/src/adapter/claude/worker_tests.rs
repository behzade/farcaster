...
fn cli_launch_and_image_envelopes_are_source_typed() {
    ...
    assert!("claude-acp".parse::<crate::Backend>().is_err());
}
...
fn access_modes_preserve_claude_model_auto_support() {
    for support in [None, Some(false), Some(true)] {
        ...
        std::fs::write(directory.path().join("claude-fixture"), script)
            .expect("test operation should succeed");
        let mut session = session(&command, directory.path());
        let model: crate::extensions::Model = serde_json::from_value(session.models[0].clone())
            .expect("test operation should succeed");
        let modes = crate::available_access_modes(BACKEND, Some(&model), None);
        assert_eq!(
...
fn queued_batch_round_trip(interrupt: bool) {
    ...
    let png = crate::extensions::PromptImage::new("YWJj".into(), "image/png".into());
    let jpeg = crate::extensions::PromptImage::new("ZGVm".into(), "image/jpeg".into());
    session
        .submit_prompt(
            "steer-1".into(),
            "same".into(),
...
fn main_session_applies_claude_steering_as_an_interrupting_handoff() {
    ...
    use crate::extensions::{ExtensionUiResponse, PromptMode};
