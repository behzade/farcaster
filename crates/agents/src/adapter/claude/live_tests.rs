...
fn settled(session: &mut dyn SessionTransport, expected: &str, tool: bool) -> Result<Turn, String> {
    ...
    while Instant::now() < deadline {
        match session.poll() {
            ...
            Some(SessionEvent::Response(crate::SessionResponse {
                result: Err(error), ..
            })) => {
...
fn real_claude_text_followup_tool_and_resume() -> Result<(), String> {
    ...
    session.send(SessionCommand::Prompt {mode:PromptMode::Normal,
        ...
        images:vec![crate::extensions::PromptImage::new("iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAIAAAD8GO2jAAAAKklEQVR4nGP4EKBBU8QwasGoBaMWjFowasGoBaMWjFowasGoBaMWDBULACvxoEydbL2eAAAAAElFTkSuQmCC".into(), "image/png".into())]})?;
...
fn real_claude_interrupt_and_recover() -> Result<(), String> {
    use crate::{WorkerActivity, WorkerEvent, WorkerSendMode};
    let project = tempfile::tempdir().map_err(|error| error.to_string())?;
