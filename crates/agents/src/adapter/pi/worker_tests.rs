...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let image = crate::extensions::PromptImage::new("aGVsbG8=".into(), "image/png".into());
...
fn worker_input_maps_only_interactive_requests() {
    ...
    assert!(
        worker_input(ExtensionUiRequest::Notify {
            id: "notice".into(),
            message: "done".into(),
            tone: crate::extensions::NotifyTone::Info,
        })
...
fn worker_factory_applies_launch_proxy_and_access_mode() -> Result<(), Box<dyn std::error::Error>> {
    ...
    command.access_mode = crate::HarnessAccessMode::Full;
