...
use farcaster_projects::trust_store::{canonical, nearest_decision};
...
pub fn startup_trust(project: &Path) -> Result<StartupTrust, String> {
...
pub fn apply(project: &Path, choice: TrustChoice) -> Result<AppliedTrust, String> {
    let trust_path = trust_environment(project)?.agent_dir.join("trust.json");
    farcaster_projects::apply(&trust_path, project, choice)
}

pub fn saved_decision(project: &Path) -> Result<Option<(PathBuf, bool)>, String> {
    nearest_decision(
