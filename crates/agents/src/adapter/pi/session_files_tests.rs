...
fn official_path_parent_resolves_to_id_and_keeps_live_hierarchy() -> TestResult {
    ...
    let cached = SessionSummary::from_cached_for_harness(
        "cached-child".into(),
        Backend::Pi,
        child_path,
...
fn discovery_carries_transient_agent_activity_without_persisting_it() -> TestResult {
    ...
    assert_eq!(
        activity.lifecycle,
        farcaster_sessions::activity::AgentLifecycle::Completed(
            farcaster_sessions::activity::AgentOutcome::Complete
        )
    );
    Ok(())
}
...
fn subagent_path_detection_distinguishes_roots_from_descendants() {
    let root = SessionSummary::from_cached_for_harness(
        "root".into(),
        Backend::Pi,
