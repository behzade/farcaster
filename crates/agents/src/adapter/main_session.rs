...
impl WorkerSessionTransport {
    pub(super) fn new(
        ...
        let selection = worker
            .model_selection()
            .unwrap_or_else(|| crate::WorkerModelSelection {
                model: history
                    .as_ref()
    ...
    fn enqueue_activity(&mut self, worker_activity: WorkerActivity) {
        let event = match worker_activity {
            ...
            WorkerActivity::ChildSessionsChanged {
                ...
                let execution = execution.unwrap_or_else(|| crate::WorkerModelSelection {
...
pub fn launch_session_locator(launch: &crate::SessionLaunch) -> Option<String> {
    match &launch.start {
        crate::SessionStart::New => launch.session_id.clone(),
        crate::SessionStart::Resume(path) | crate::SessionStart::Fork(path) => {
