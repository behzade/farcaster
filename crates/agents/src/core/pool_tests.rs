...
use super::*;
use crate::contract::{StartWorker, WorkerContext, WorkerInputResponse, WorkerStatus};
use crate::{WorkerEvent, WorkerLaunch, WorkerSendMode, WorkerSession, WorkerSessionFactory};
...
struct FakeSession {
    ...
    closes: Arc<std::sync::atomic::AtomicUsize>,
    drops: Arc<std::sync::atomic::AtomicUsize>,
    identity: Option<crate::CallerIdentity>,
    fail_close: Arc<std::sync::atomic::AtomicBool>,
    fail_send: Arc<std::sync::atomic::AtomicBool>,
    fail_send_on: Arc<std::sync::atomic::AtomicUsize>,
    send_count: Arc<std::sync::atomic::AtomicUsize>,
...
fn future_launches_use_current_proxy_and_requested_access_mode() -> Result<(), String> {
    ...
    first.access_mode = crate::HarnessAccessMode::Sandboxed;
    pool.start(first)?;

    pool.set_app_proxy(None)?;
    let mut second = request(project.path());
    second.name = "after-clear".into();
    second.access_mode = crate::HarnessAccessMode::Full;
    pool.start(second)?;

    let launches = factory.launches.lock().map_err(|_| "launches")?;
    assert_eq!(launches[0].app_proxy.as_deref(), Some(proxy));
    assert_eq!(launches[0].access_mode, crate::HarnessAccessMode::Sandboxed);
    assert_eq!(launches[1].app_proxy, None);
    assert_eq!(launches[1].access_mode, crate::HarnessAccessMode::Full);
    Ok(())
}
...
fn worker_parent_reports_and_inputs_follow_a_replacement_parent_process() -> Result<(), String> {
    ...
    events
        .send(WorkerEvent::NeedsInput(crate::WorkerInput {
            id: "approval".into(),
            prompt: "Proceed?".into(),
