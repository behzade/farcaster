...
fn generated_names_have_a_large_namespace_and_valid_shape() {
    ...
    assert!(crate::valid_worker_name(&name));
...
fn worker_chosen_names_are_bounded_identifiers() {
    assert!(crate::valid_worker_name("diff-review"));
    assert!(crate::valid_worker_name("Auth_Tests2"));
    assert!(!crate::valid_worker_name(""));
    assert!(!crate::valid_worker_name("two words"));
    assert!(!crate::valid_worker_name("-review"));
    assert!(!crate::valid_worker_name(&"a".repeat(49)));
}
