...
pub use edit::{RepositoryEdit, RepositoryEditReview};
pub use preferences::{PreferenceStore, load as load_preferences, save as save_preferences};
...
impl RepositoryBackend {
    ...
    pub const fn location(&self) -> &RepositoryLocation {
        &self.location
    }
    ...
    #[cfg(test)]
    pub fn load_diff(&self, target: DiffTarget) -> Result<DiffResult, RepositoryError> {
        self.validate_target(&target)?;
        let _operation = repository_operation()?;
        self.operations.load_diff(self, target)
    }

    pub fn list_project_files(&self) -> Result<Vec<String>, RepositoryError> {
        let _operation = repository_operation()?;
