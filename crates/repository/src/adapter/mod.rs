...
impl RepositoryBackend {
    ...
    pub fn jj_init_required(location: &RepositoryLocation) -> Result<bool, RepositoryError> {
        Ok(location.kind == RepositoryKind::Git
            && !marker_exists(&location.workspace_root.join(".jj"))?)
    }

    pub fn init_jj_colocated(repository: &Path) -> Result<(), RepositoryError> {
