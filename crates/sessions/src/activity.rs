...
pub fn agent_activity_key(path: &std::path::Path) -> String {
    crate::normalize_session_path(path)
        .to_string_lossy()
        .into_owned()
}
...
impl AgentActivity {
    pub fn limited_fallback(session: &SessionSummary) -> Self {
        Self {
...
impl ActivityBuilder {
    pub fn observe_entry(&mut self, entry: &Value) {
