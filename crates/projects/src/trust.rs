...
pub const TRUST_DESCRIPTION: &str = "Trusting allows Farcaster to run repository commands in this folder. Agent settings and extensions follow each backend's own trust rules.";

pub fn startup_trust(store: &Path, project: &Path) -> Result<StartupTrust, String> {
...
pub fn repository_execution_allowed(store: &Path, project: &Path) -> Result<bool, String> {
    Ok(saved_decision(store, project)?.is_some_and(|(_, trusted)| trusted))
}

pub fn saved_decision(store: &Path, project: &Path) -> Result<Option<(PathBuf, bool)>, String> {
    nearest_decision(store, project)
}

pub fn options(project: &Path) -> Vec<TrustOption> {
...
pub fn apply(store: &Path, project: &Path, choice: TrustChoice) -> Result<AppliedTrust, String> {
