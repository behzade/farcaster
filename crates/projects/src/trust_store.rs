...
pub fn nearest_decision(path: &Path, project: &Path) -> Result<Option<(PathBuf, bool)>, String> {
...
pub fn update_trust_file(path: &Path, updates: &[(PathBuf, Option<bool>)]) -> Result<(), String> {
    let _lock = TrustFileLock::acquire(path)?;
