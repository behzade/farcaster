...
impl<T: Clone> PersistentVec<T> {
    ...
    pub fn iter_rev(&self) -> impl DoubleEndedIterator<Item = &T> + ExactSizeIterator {
    ...
    pub fn splice(&mut self, range: Range<usize>, values: impl IntoIterator<Item = T>) {
        assert!(range.start <= range.end && range.end <= self.len());
        let next = {
            let mut cursor = self.tree.cursor::<Count>(());
    ...
    pub fn rposition(&self, mut predicate: impl FnMut(&T) -> bool) -> Option<usize> {
