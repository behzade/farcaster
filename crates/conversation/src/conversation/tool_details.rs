...
use farcaster_agent_protocol::{CommonTool, ToolCategory, ToolMetadata};
...
impl ToolDetails {
    ...
    pub fn write_inspection(&self, mut writer: impl Write) -> std::io::Result<()> {
        write!(writer, "Tool: {}\n\nArguments:\n", self.name)?;
...
impl TranscriptItem {
    pub fn tool_execution_state(&self) -> Option<ToolExecutionState> {
        if self.is_error {
