...
fn state_path() -> Result<PathBuf, String> {
    std::env::var_os("FARCASTER_DATA_DIR")
        .map(|root| PathBuf::from(root).join("state.sqlite3"))
        .ok_or_else(|| "FARCASTER_DATA_DIR is required for live tests".to_owned())
}
...
fn assert_isolated_state() -> Result<(), String> {
    ...
    farcaster_storage::StateStore::open_at(&expected)?;
    Ok(())
}
...
fn pending_outbox_messages(session: &Path, expected: &[&str]) -> Result<Vec<String>, String> {
    let database = state_path()?;
    let store = farcaster_storage::StateStore::open_at(&database)?;
    Ok(store
        .queued_prompts()?
        .into_iter()
        .filter(|prompt| {
