...
impl RuntimeHandle {
    ...
    pub fn spawn(
        ...
        Self::spawn_with_configuration_refresh(project, draft, initial_session, command, host, true)
    }
    ...
    pub fn wake_receiver(&self) -> async_channel::Receiver<()> {
        self.wake.clone()
    }
}
...
fn persist_configurations(
    state: Option<&farcaster_storage::SharedStateStore>,
...
struct Supervisor {
    ...
    configurations: HarnessConfigurationStore,
    catalog_state: Option<farcaster_storage::SharedStateStore>,
...
impl Supervisor {
    fn new(
        ...
        let catalog_state = host.state_store().ok();
