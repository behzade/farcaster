...
use crate::runtime::{RuntimeCommand, tests::owner_without_process};
...
fn command_entry_sends_all_unacknowledged_inputs_before_first_escape() -> Result<(), String> {
    ...
    let reopened = farcaster_storage::StateStore::open_at(&temp.path().join("state.sqlite3"))?;
...
-> Result<(), String> {
    ...
    let reopened =
        farcaster_storage::StateStore::open_at(&failure_temp.path().join("state.sqlite3"))?;
