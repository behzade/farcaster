...
stdenvNoCC.mkDerivation {
  ...
  installPhase = ''
    runHook preInstall
    mkdir -p $out
    cp background-jobs.ts broker-client.ts broker-policy.ts denial-summary.ts index.ts native-background-jobs.ts native-network-proxy.ts network-policy.ts sandbox-config.ts development-caches.ts io-permissions.ts io-policy.ts native-sandbox-ops.ts permission-system-approval.ts project-policy-store.ts project-policy.ts tool-schemas.ts package.json $out/
