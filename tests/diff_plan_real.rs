use farcaster::diff_plan::{DiffLayout, DiffPlanOptions, DiffPlanRow, DiffRenderPlan, plan_patch};
...
fn stress_plan_handles_pierres_real_multi_commit_patch() {
    ...
    assert_eq!(
        plan.files
            .iter()
            .filter(|file| file.kind == farcaster::diff_plan::FileChangeKind::Renamed)
            .count(),
        13
    );
    assert!(plan.files.iter().any(|file| {
        file.kind == farcaster::diff_plan::FileChangeKind::Renamed && file.rows.is_empty()
    }));
    assert!(plan.files.iter().any(|file| {
        file.kind == farcaster::diff_plan::FileChangeKind::Deleted
            && file.path.is_none()
