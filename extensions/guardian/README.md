...
The reviewer uses `openai-codex/gpt-5.6-terra` with low reasoning by default.
It receives a bounded user and assistant transcript as untrusted evidence and
returns:
...
The extension fails closed if Terra is missing or review fails without an
...
Guardian sends the whole sibling set to Terra once and keeps each verdict by
tool-call ID.

Home Manager writes the active reviewer mode, model, reasoning, and repeat limit to
...
After Terra approves the same exact action twice in one session, Pi asks whether
